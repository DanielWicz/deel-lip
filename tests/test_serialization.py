...
def test_spectral_conv2d_roundtrip_with_mirrored_strategy(tmp_path):
    ...
    sample = np.random.default_rng(4321).standard_normal((2, 8, 8, 3)).astype(
        np.float32
    )
    original = model.predict(sample)

    target_path = tmp_path / "spectral_conv_mirrored.keras"
    model.save(target_path)
